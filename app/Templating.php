...
return [
    ...
    // site url
    '{Site_url}' => '<?php echo AntCMS::urlBase();?>',
    '{Site_current}' => '<?php echo AntCMS::UrlCurrent();?>',
    // pagination for other folder of content not blog
    '{Pages: ([^}]*)}' => '<?php echo AntCMS::actionRun("articles",["$1",AntCMS::$config["pagination"]]);?>',
    // comment
    //{* comment *}
    '{\*(.*?)\*}' => '<?php echo "\n";?>',
    // confitional
    ...
    '{\/If}' => '<?php endif; ?>',
    // segments
    '{Segment: ([^}]*)}' => '<?php if (AntCMS::urlCurrent() == "$1"): ?>',
    '{\/Segment}' => '<?php endif; ?>',
    // loop
    ...
    // actions
    '{Action: ([a-zA-Z\-\._\[\]\'"0-9]+)}' => '<?php AntCMS::actionRun(\'$1\'); ?>',
    // include
    '{Include: (.+?\.[a-z]{2,4})}' => '<?php include_once(ROOT."/$1"); ?>',
    // blocks
    '{Block: ([a-zA-Z\-\._\[\]\'"0-9]+)}' => '<?php echo AntCMS::getFile(ROOT."/public/blocks/$1.html"); ?>',
    // partial
    '{Partial: (.+?\.[a-z]{2,4})}' => '<?php include_once(ROOT."/public/views/partials/$1"); ?>',
    // assets
    '{Assets: (.+?\.[a-z]{2,4})}' => '<?php echo AntCMS::urlBase()."/public/$1" ?>',
    // Style css
